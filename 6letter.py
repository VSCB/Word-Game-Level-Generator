...
six_letter_words_nltk = [word for word in word_list if len(word) == 6]
six_letter_words_nltk.sort(key=lambda w: -freq_dist[w])  # Sort by decreasing frequency

def generate_word_list():
    if not six_letter_words_nltk:
        return None

    base_word = six_letter_words_nltk.pop(0)  # Get and remove the most common word from the list
    ...
    six_letter_combinations = set([''.join(comb) for comb in itertools.permutations(base_word, 6)])
    ...
    six_letter_words = filter_common_words(word_list, six_letter_combinations, freq_dist)
    ...
    return {
        ...
        "6-letter words": ', '.join(six_letter_words)
    }
...
def main():
    ...
    df.to_excel("six_letter_word_lists.xlsx", index=False)
